...
点击“生成本批报告”会为当前候选池逐只创建日线 Report V2 任务，页面立即显示
排队状态并每两秒刷新进度；刷新页面后会继续查询尚未结束的批次。
查询中断时保留已有结果，并提供恢复查询入口；恢复查询不会重新创建报告任务。
投顾报告与交易日记互不共用数据：前者是自选池结构分析，后者是成交账本与周期复盘。
投顾报告按“查询候选 → 观察进度 → 查看报告”展示。报告页以当前股票为标题，
只保留一套行情摘要，并优先展示结构图；手机端导航和日记主区按屏幕宽度排布。
已完成或降级的报告可通过股票选择器和“上一只 / 下一只”连续阅读，切换股票保留当前日线或周线。
...
不会包含操作按钮。分享页提供摘要、结构图、情景、风险、证据及兑现结果的章节导航，
导航不改变分享链接，也不会出现在导出文件中。
...
交易日记按“交易日历 / 记账与日复盘 / 周期报告”组织入口，日历内选择月、周视图，
周期报告内选择季报、年报；月周复盘入口保留在对应日历下方。
点击月份标题选择月份，也可用左右箭头翻页或回到今天。
点击“记一笔 / 写复盘”会定位到所选日期的对应编辑区。
日期明细支持键盘 Tab 切换与 Escape 关闭，关闭后恢复日历或导航焦点。
日复盘在当前日记页面按日期保留未保存草稿，补录成交或刷新账户数据不会覆盖正在编辑的内容。
BS 标注按股票、周期和 K 线保留草稿，保存时显示进度并防止重复提交。
草稿不会跨页面刷新持久化，刷新页面或离开日记前仍需保存。
账户与当日数据加载失败时可单独重试；周期报告查询中断后可恢复，创建失败会明确提示。
...
读取手标评论。点击 K 线后可新增 BS 分析，也可修改或删除该柱已有分析；点击该柱
已使用的点位类型会直接载入原分析，后续保存只更新原记录，不会重复新增。点位类型
支持新增自定义类型。30 分钟行情失败时仍可使用日线 BS 分析。
